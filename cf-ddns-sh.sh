...
# script commit for sh not the bash
# because sh can not handle array so only single hostname updates possible.
...
if [ "$myCFRECORD_NAMES" != "$myCFZONE_NAME" ] && ! [ -z "${myCFRECORD_NAMES##*$myCFZONE_NAME}" ]; then
  myCFRECORD_NAMES="$myCFRECORD_NAMES.$myCFZONE_NAME"
  echo " => Hostname is not a FQDN, assuming $myCFRECORD_NAMES"
fi
...
if [ -f $ID_FILE ] && [ $(wc -l $ID_FILE | cut -d " " -f 1) == 2 ] && [ "$(sed -n '1,1p' "$ID_FILE")" == "$myCFZONE_NAME" ]; then
  ...
  if [ "$myCFMODE" = "update" ] && [ "$myCFMODE" != "create" ]; then
    myCFRECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$myCFZONE_ID/dns_records?name=$myCFRECORD_NAMES" -H "X-Auth-Email: $myCFUSER" -H "X-Auth-Key: $myCFKEY" -H "Content-Type: application/json" | grep -Po '(?<="id":")[^"]*' | head -1 )
  fi
fi
# update cloudflare DNS, trying to update DNS if entry exists
echo "Processing $myCFRECORD_NAMES"
if [ "$myCFMODE" = "update" ] && [ "$myCFMODE" != "create" ]; then

  # update record
  echo "Updating DNS $myCFRECORD_TYPE Record : $myCFRECORD_NAMES with $myWAN_IP"
  myRESPONSE=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$myCFZONE_ID/dns_records/$myCFRECORD_ID" \
    ...
    --data "{\"id\":\"$myCFZONE_ID\",\"type\":\"$myCFRECORD_TYPE\",\"name\":\"$myCFRECORD_NAMES\",\"content\":\"$myWAN_IP\", \"ttl\":$myCFTTL}")
  else

  # create new record
  echo "Creating DNS $myCFRECORD_TYPE Record : $myCFRECORD_NAMES with $myWAN_IP"
  myRESPONSE=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$myCFZONE_ID/dns_records/" \
    -H "X-Auth-Email: $myCFUSER" \
    -H "X-Auth-Key: $myCFKEY" \
    -H "Content-Type: application/json" \
    --data "{\"type\":\"$myCFRECORD_TYPE\",\"name\":\"$myCFRECORD_NAMES\",\"content\":\"$myWAN_IP\", \"ttl\":$myCFTTL}")
  fi

# check for success
if [ "$myRESPONSE" != "${myRESPONSE%success*}" ] && [ "$(echo $myRESPONSE | grep "\"success\":true")" != "" ]; then
  echo "Updated succesfuly with following DATA!"
  echo "{\"id\":\"$myCFZONE_ID\",\"type\":\"$myCFRECORD_TYPE\",\"name\":\"$myCFRECORD_NAMES\",\"content\":\"$myWAN_IP\", \"ttl\":$myCFTTL}"
else
  echo 'Something went wrong :('
  echo "Response: $myRESPONSE"
fi
echo "Processed: $myCFRECORD_NAMES"
